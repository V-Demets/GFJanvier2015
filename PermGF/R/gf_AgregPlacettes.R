...
gf_AgregMoySdEr <- function(df, Regroup="Strate", var.sup="") {
  ...
  fmla <- paste(paste(fmla1,fmla2,"PoidsPlacette", sep=" + "), " ~ NumForet + Regroup + Cycle")
  if (var.sup=="") {
  ...
  TabSum[,debut:(debut-1+ncol)] <- TabSum[,debut:(debut-1+ncol)]/TabSum$Poids
  b <- b/TabSum[,debut:(debut-1+ncol)]
  TabSum[,(debut+ncol):(debut+2*ncol)] <- b * 100
  TabSum$PoidsPlacette <- NULL
  Nbre <- summaryBy(PoidsPlacette ~ Regroup, data=t1, FUN=length)
  names(Nbre)[2] <- "Nbre"
  b <- qt(0.975, Nbre$Nbre)* b/Nbre$Nbre^0.5 * 100
  TabSum <- cbind(TabSum, b)
  TabSum <- merge(TabSum, Nbre, by="Regroup", all=T)
  TabSum
}

gf_AgregMoySdEr <- function(df, Regroup="Strate", var.sup=NULL,debut=4, ncol=3) {
...
Titre <- c("Nha","Gha","Vha","AcctV","Nha_cv","Gha_cv","Vha_cv","AcctV_cv","Nha_er","Gha_er","Vha_er","AcctV_er")
tab <- gfPlaDen[,c(1:2,4:7,11)]
gfDendroForet <- gf_AgregMoySdEr(tab,"Foret")
names(gfDendroForet)[4:15] <- Titre
gfDendroGroupe <- gf_AgregMoySdEr(tab,"Groupe")
names(gfDendroGroupe)[4:15] <- Titre
gfDendroStrate <- gf_AgregMoySdEr(tab,"Strate")
names(gfDendroStrate)[4:15] <- Titre
gfDendroStation <- gf_AgregMoySdEr(tab,"Station")
names(gfDendroStation)[4:15] <- Titre
gfDendroTypo <- gf_AgregMoySdEr(tab,"Typologie")
names(gfDendroTypo)[4:15] <- Titre
gfDendroGroupe1 <- gf_AgregMoySdEr(tab,"Groupe1")
names(gfDendroGroupe1)[4:15] <- Titre
gfDendroGroupe2 <- gf_AgregMoySdEr(tab,"Groupe2")
names(gfDendroGroupe2)[4:15] <- Titre

# ---------- Agregation économie ----------
Titre <- c("VcHa","VpHa","Gain","VcHa_cv","VpHa_cv","Gain_cv","VcHa_er","VpHa_er","Gain_er")
gfEcoForet <- gf_AgregMoySdEr(gfPla[,c(1:2,4,8:10)],"Foret")
names(gfEcoForet)[4:12] <- Titre
gfEcoGroupe <- gf_AgregMoySdEr(gfPla[,c(1:2,4,8:10)],"Groupe")
names(gfEcoGroupe)[4:12] <- Titre
gfEcoStrate <- gf_AgregMoySdEr(gfPla[,c(1:2,4,8:10)],"Groupe")
names(gfEcoStrate)[4:12] <- Titre
gfEcoStation <- gf_AgregMoySdEr(gfPla[,c(1:2,4,8:10)],"Groupe")
names(gfEcoStation)[4:12] <- Titre
gfEcoTypo <- gf_AgregMoySdEr(gfPla[,c(1:2,4,8:10)],"Groupe")
names(gfEcoTypo)[4:12] <- Titre
gfEcoGroupe1 <- gf_AgregMoySdEr(gfPla[,c(1:2,4,8:10)],"Groupe")
names(gfEcoGroupe1)[4:12] <- Titre
gfEcoGroupe2 <- gf_AgregMoySdEr(gfPla[,c(1:2,4,8:10)],"Groupe")
names(gfEcoGroupe2)[4:12] <- Titre
